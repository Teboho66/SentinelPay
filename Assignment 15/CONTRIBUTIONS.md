# CONTRIBUTIONS.md - Assignment 15 Submission

**Author:** Teboho Mokoni  
**Repository:** https://github.com/Teboho66/SentinelPay  
**Assignment:** 15 - Cross-Project Contributions & Collaborative Development

---

## Quick Links

| Deliverable | File | Status |
|---|---|---|
| Contribution Plan | [CONTRIBUTION_PLAN.md](./CONTRIBUTION_PLAN.md) | ✅ Complete |
| Pull Requests submitted | [MERGED_PRS.md](./MERGED_PRS.md) | ⏳ Update with PR links |
| Reflection | [REFLECTION.md](./REFLECTION.md) | ✅ Complete |

---

## My Repository as a Contribution Target

SentinelPay received **28 ⭐ stars** and **36 🍴 forks** from classmates
in Assignment 14, making it one of the most-forked projects in the cohort.

What made it fork-friendly:
- Live Swagger UI at `/docs` - contributors can explore the API without reading code
- 288 passing tests - instant confidence that nothing is broken
- `CONTRIBUTING.md` with step-by-step setup from scratch
- `ROADMAP.md` with 8 labeled `good-first-issue` tasks ready to pick up
- MIT License - no legal barriers to contribution

---

## Contributions Made to Peers' Repositories

> Fill in the table below with your actual PR links after submitting.
> Each row = one PR submitted to a classmate's repo.

| # | Classmate Repo | Issue | PR Link | Type | Status |
|---|---|---|---|---|---|
| 1 | [TailorFit](https://github.com/znxos/TailorFit) | [Issue #40 – API Parameters - 4](https://github.com/znxos/TailorFit/issues/40) | [PR #1 – Add API key request and response schemas](PASTE_PR_LINK_HERE) | `feature-request`, `REST API`, `good-first-issue` | ⏳ Submitted |
| 2 | [repo link] | [issue link] | [PR link] | `good-first-issue` | 📝 Planned |
| 3 | [repo link] | [issue link] | [PR link] | `feature-request` | 📝 Planned |

---

## Contribution Stats

| Metric | Count |
|---|---:|
| PRs submitted | 1 / 3 |
| PRs merged | 0 / 3 - update after merges |
| Repositories contributed to | 1 / 3 |
| Feature-request PRs | 1 |
| Local test suites passed | 1 |
| TailorFit tests passed | 32 passed, 1 warning |
| Stars received on SentinelPay | 28 |
| Forks received on SentinelPay | 36 |

---

## Key Takeaway

Open-source contribution is 20% code and 80% communication. The PR that
gets merged is the one with a clear title, a description that explains
*why* not just *what*, a linked issue, and passing CI. The code itself
rarely gets rejected - it is usually the context around it that determines
whether a maintainer has enough confidence to click Merge.

---

*SentinelPay - Because every millisecond between a transaction and a fraud signal costs money.*