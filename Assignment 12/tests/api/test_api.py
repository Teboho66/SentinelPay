...
class TestTransactionAPI:
    ...
    def test_apply_decision_returns_fraud_decision(self, client):
        client.post("/api/transactions", json=TXN_PAYLOAD)
        r = client.post(
            "/api/transactions/TXN-API-001/decision",
            json={
                "fraud_score": 0.93,
...
class TestFraudCaseAPI:
    ...
    def test_assign_analyst(self, client):
        ...
        case_id = r.json()["case_id"]
        r2 = client.patch(
            f"/api/fraud-cases/{case_id}/assign", json={"analyst_id": "j.mokoena"}
        )
        assert r2.status_code == 200
    ...
    def test_resolve_confirmed(self, client):
        ...
        r2 = client.patch(
            f"/api/fraud-cases/{case_id}/resolve",
...
class TestMLModelAPI:
    ...
    def test_promote_to_staging(self, client):
        ...
        model_id = r.json()["model_id"]
        r2 = client.patch(
            f"/api/ml-models/{model_id}/promote", json={"target_stage": "STAGING"}
        )
        assert r2.status_code == 200
    ...
    def test_promote_to_production_passes_gate(self, client):
        ...
        r2 = client.patch(
            f"/api/ml-models/{model_id}/promote", json={"target_stage": "PRODUCTION"}
        )
        assert r2.status_code == 200
    ...
    def test_hot_swap_production_model(self, client):
        ...
        r2 = client.patch(
            f"/api/ml-models/{model_id}/hot-swap",
